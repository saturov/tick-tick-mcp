...
- Не предполагай наличие `TICKTICK_API_KEY` — проверяй его перед командами, но сообщай пользователю статус окружения и авторизации только при первом TickTick-запросе в треде, явном запросе диагностики или проблеме.
- Если окружение уже проверено и команды TickTick успешно выполняются, не повторяй в промежуточных сообщениях, что `.venv` есть, но не активирован, или что команды запускаются через `.venv`/`.zshrc`.
